...
Um *laço de repetição* ou *loop* permite várias execuções de uma instrução ou um conjunto de instruções. Existem dois componentes em um *loop*: a instrução de controle e o corpo do loop.
A instrução de controle gerencia a execução das instruções dependendo da condição. 
Neste componente o *loop* faz uma consulta e se a resposta exigir uma ação, ela será executada. A mesma consulta é feita repetidas vezes até que outras ações sejam tomadas.
Sempre que a consulta é feita no *loop*, ela é conhecida como uma iteração do loop.
Por sua vez, o corpo do *loop* consiste no conjunto de instruções a serem executadas.


### Laços `FOR`

O *loop* `FOR` em ``R` é uma estrutura de controle que permite executar um conjunto de instruções repetidamente com base em um número específico de iterações. 
Neste *loop* a condição de teste é verificada primeiro, sendo válida, o corpo do *loop* é executado. 
